...
def query_range(max_, min_, term):
    """ Returns range filters """
    if max_ and min_:
        q_str = '{"range": {"%s":{"gte":"%s","lte":"%s"}}},' % (term, min_, max_)
    elif min_:
        q_str = '{"range": {"%s":{"lte":"%s"}}},' % (term, min_)
    else:
        q_str = '{"range": {"%s":{"gte":"%s"}}},' % (term, max_)
    return q_str